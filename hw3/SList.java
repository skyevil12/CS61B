...
public class SList {
  ...
  public void squish() {
	...
	// Compare first and next, only link to different elements
	SListNode tmpTraverse = head.next, tmpCur = head;
	do{
		if(!tmpCur.item.equals(tmpTraverse.item)) {
			tmpCur.next = tmpTraverse;
			tmpCur = tmpCur.next;
		}
		tmpTraverse = tmpTraverse.next;
	} while(null != tmpTraverse);
	
	//Must set the tail as null
	tmpCur.next = null;
  }
  ...
  public void twin() {
	...
	// Two tmp var to traverse and change result
	SListNode tmpTraverse = head.next, tmpCur = head;
	while(null != tmpTraverse) {
		SListNode dup = new SListNode(tmpCur.item, tmpTraverse);
		tmpCur.next = dup;
		tmpCur = tmpTraverse;
	...
	SListNode dup = new SListNode(tmpCur.item);
	tmpCur.next = dup;
  }
